...
import type { SetupStepId } from '../entities/setup'
...
export default function Setup() {
  ...
  return <Screen header={header}>
    ...
    {next && <div className="mb-1 rounded-lg border border-accent-soft bg-accent-faint px-[15px] pt-3.5 pb-[15px]">
      <div className="lbl text-accent">Следующий шаг</div>
      <div className="mt-1 text-title font-semibold">{next.title}</div>
      <div className="mt-0.5 mb-3.5 text-sub leading-[1.4] text-muted">{next.sub}</div>
      <Button block onClick={() => run(next.id)}>{next.action}</Button>
    </div>}
    ...
    <Card>
      <List>
        {setup.steps.map(step => <ListRow
          ...
          onClick={step.status === 'next' || step.status === 'todo' ? () => run(step.id) : undefined}
        />)}
      </List>
    </Card>
